				...
				<div class="sleft">
					<form name="formSearch" id="formSearch" method="get" name="form1">
						<select style="width: 160px; float: left; margin-right: 10px;" name="market" class="form-control">
							...
							>交易市场</option>
							<?php if(is_array($market)): $i = 0; $__LIST__ = $market;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['name']); ?>"
						...
						<select style="width: 160px; float: left; margin-right: 10px;" name="status" class="form-control">
							...
							<option value="1" <?php if(($_GET['status']) == "1"): ?>selected<?php endif; ?>>上传截图</option>
							<option value="2" <?php if(($_GET['status']) == "2"): ?>selected<?php endif; ?>>等待买家确认</option>
							<option value="3" <?php if(($_GET['status']) == "3"): ?>selected<?php endif; ?>>已完成</option>
							<option value="-1" <?php if(($_GET['status']) == "-1"): ?>selected<?php endif; ?>>已撤销</option>
							<option value="5" <?php if(($_GET['status']) == "4"): ?>selected<?php endif; ?>>争议订单</option>
						</select>
		...
		<div class="data-table table-striped">
			<table class="">
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<?php if(empty($vo["img"])): ?>等待上传
									<?php else: ?>

										<img onclick="chakan(id)" id="<?php echo ($vo["id"]); ?>" src="<?php echo ($vo["img"]); ?>" width="50px" height="100px"><?php endif; ?>
							...
							<td>
								...
								<?php if(($vo["status"]) == "1"): ?>上传截图<?php endif; ?>
								<?php if(($vo["status"]) == "2"): ?>等待确认<?php endif; ?>
								<?php if(($vo["status"]) == "3"): ?>已完成<?php endif; ?>
								<?php if(($vo["status"]) == "4"): ?>争议<?php endif; ?>
								<?php if(($vo["status"]) == "5"): ?>处理争议中<?php endif; ?>
								<?php if(($vo["status"]) == "-1"): ?>已撤销<?php endif; ?>
							</td>
							<td>
								<?php if(($vo["status"]) == "0"): ?><a href="<?php echo U('Trade/seeklog_status?id='.$vo['id'].'&action=chexiao');?>" class="ajax-get btn btn-primary btn-xs">撤销 </a><?php endif; ?>
								<?php if(($vo["status"]) == "4"): ?><a href="<?php echo U('Trade/seeklog_status?id='.$vo['id'].'&action=chuli');?>" class="ajax-get btn btn-primary btn-xs">处理 </a><?php endif; ?>
								<?php if(($vo["status"]) == "5"): ?><a href="<?php echo U('Trade/seeklog_status?id='.$vo['id'].'&action=chexiao');?>" style="background-color: red" class="ajax-get btn btn-primary btn-xs">撤销 </a>
									<a href="<?php echo U('Trade/seeklog_status?id='.$vo['id'].'&action=wancheng');?>" class="ajax-get btn btn-primary btn-xs">交易完成 </a><?php endif; ?>
							</td>
...
//			});
	...
	<script type="text/javascript" charset="utf-8">
		//导航高亮
		highlight_subnav("<?php echo U('Trade/seek');?>");
	</script>