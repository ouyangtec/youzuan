...
<script>
    ...
    function tradeadd_buy() {
        ...
        var  pay =mum/100*(100 + parseInt(buy_fee));
    ...
    function tradeadd_sell() {
        ...
        var  pay =mum/100*(100 - parseInt(sell_fee));
        layer.confirm('您卖出单价:'+price+',数量:'+num+' '+xnb+"</br>可收入"+pay+" "+rmb+',是否确定?', {
            btn: ['确定','取消'], //按钮
            closeBtn: 0,
            btn2: function(index){
                layer.close(index);
                    ...
                    if (data['depth']) {
                        ...
                        if (data['depth']['sell']) {
                            for (i = 0; i < data['depth']['sell'].length; i++) {
                                list+='<li><span class="serial red">卖'+(sellk - i)+'</span>\n' +
