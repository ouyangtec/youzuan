...
class ShopController extends AdminController
{
	...
	public function chexiao($id = NULL)
	{
		...
		$mo->execute('lock tables btchanges_user_coin write,btchanges_shop_log write');
		$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $shoplog['userid']))->setInc($shoplog['coinname'], $shoplog['xuyao']);
		$rs[] = $mo->table('btchanges_shop_log')->where(array('id' => $id))->save(array('status' => 2));
