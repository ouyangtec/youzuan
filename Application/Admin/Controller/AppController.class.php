...
class AppController extends AdminController
{
	...
	public function ads_user()
	{
		...
		$where = array();
		$count = $this->Model->join('btchanges_user ON btchanges_user.id = btchanges_app_vipuser.uid')->join('btchanges_app_vip ON btchanges_app_vip.id = btchanges_app_vipuser.vip_id')->field('btchanges_user.username,btchanges_app_vipuser.*,btchanges_app_vip.name as vip_name,btchanges_app_vip.tag')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = $this->Model->join('btchanges_user ON btchanges_user.id = btchanges_app_vipuser.uid')->join('btchanges_app_vip ON btchanges_app_vip.id = btchanges_app_vipuser.vip_id')->field('btchanges_user.username,btchanges_app_vipuser.*,btchanges_app_vip.name as vip_name,btchanges_app_vip.tag')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$this->assign('list', $list);
