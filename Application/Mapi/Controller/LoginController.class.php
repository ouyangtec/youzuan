...
class LoginController extends CommonController
{
	...
	public function submit($username, $password)
	{
		...
		$rs = array();
		$rs[] = $mo->table('btchanges_user')->where(array('id' => $user['id']))->setInc('logins', 1);
		$rs[] = $mo->table('btchanges_user_log')->add(array('userid' => $user['id'], 'type' => 'APP登录', 'remark' => $remark, 'addtime' => time(), 'addip' => get_client_ip(), 'addr' => get_city_ip(), 'status' => 1));
	...
	public function findpwd()
	{
		if (IS_POST) {
			...
			$mo->execute('lock tables btchanges_user write , btchanges_user_log write ');
			$rs = array();
			$rs[] = $mo->table('btchanges_user')->where(array('id' => $user['id']))->save(array('password' => md5($input['password'])));
	...
	public function findpaypwd()
	{
		if (IS_POST) {
			...
			$mo->execute('lock tables btchanges_user write , btchanges_user_log write ');
			$rs = array();
			$rs[] = $mo->table('btchanges_user')->where(array('id' => $user['id']))->save(array('paypassword' => md5($input['password'])));
	...
	public function reg($username, $mobile, $password, $invit, $truename, $idcard, $paypassword, $reg_code)
	{
		...
		$rs = array();
		$rs[] = $mo->table('btchanges_user')->add(array('username' => $username, 'password' => md5($password), 'invit' => $tradeno, 'idcard' => $idcard, 'moble' => $mobile, 'truename' => $truename, 'paypassword' => $paypassword, 'tpwdsetting' => 1, 'invit_1' => $invit_1, 'invit_2' => $invit_2, 'invit_3' => $invit_3, 'addip' => get_client_ip(), 'addr' => get_city_ip(), 'addtime' => time(), 'status' => 1));
		$rs[] = $mo->table('btchanges_user_coin')->add(array('userid' => $rs[0]));
