...
class TradeController extends CommonController
{
	...
	public function getTradedata($marketid = NULL)
	{
		...
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from btchanges_trade where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit 5;');
		$sell = array_reverse($mo->query('select id,price,sum(num-deal)as nums from btchanges_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit 5;'));
	...
	public function upTrade($paypassword = NULL, $marketid = NULL, $price, $num, $type)
	{
		...
		if (C('coin')[$xnb]['fee_bili']) {
			if ($type == 2) {
				...
				if ($bili_user) {
					...
					if ($bili_keyi) {
						$bili_zheng = M()->query('select id,price,sum(num-deal)as nums from btchanges_trade where userid=' . $uid . ' and status=0 and type=2 and market like \'%' . $xnb . '%\' ;');
		...
		$mo->execute('lock tables btchanges_trade write ,btchanges_user_coin write');
		$rs = array();

		if ($type == 1) {
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $uid))->setDec($rmb, $mum);
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $uid))->setInc($rmb . 'd', $mum);
			$rs[] = $mo->table('btchanges_trade')->add(array('userid' => $uid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 1, 'addtime' => time(), 'status' => 0));
		}
		else if ($type == 2) {
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $uid))->setDec($xnb, $num);
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $uid))->setInc($xnb . 'd', $num);
			$rs[] = $mo->table('btchanges_trade')->add(array('userid' => $uid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
	...
	public function matchingTrade($market = NULL)
	{
		...
		$mo = M();
		$mo->table('btchanges_trade')->where('num<deal')->setField('status', 1);
		$new_trade_btchanges = 0;

		for (; true; ) {
			$buy = $mo->table('btchanges_trade')->where(array('market' => $market, 'type' => 1, 'status' => 0))->order('price desc,id asc')->find();
			$sell = $mo->table('btchanges_trade')->where(array('market' => $market, 'type' => 2, 'status' => 0))->order('price asc,id asc')->find();
			...
			if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
				...
				$mo->execute('set autocommit=0');
				$mo->execute('lock tables btchanges_trade write ,btchanges_trade_log write ,btchanges_user write,btchanges_user_coin write,btchanges_invit write');
				$rs[] = $mo->table('btchanges_trade')->where(array('id' => $buy['id']))->setInc('deal', $amount);
				$rs[] = $mo->table('btchanges_trade')->where(array('id' => $sell['id']))->setInc('deal', $amount);
				$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->setInc($xnb, $amount);
				$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->setDec($rmb . 'd', $save_buy_rmb);
				$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $sell['userid']))->setInc($rmb, $sell_save);
				$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $sell['userid']))->setDec($xnb . 'd', $save_sell_xnb);
				$rs[] = $mo->table('btchanges_trade_log')->add(array('userid' => $buy['userid'], 'peerid' => $sell['userid'], 'market' => $market, 'price' => $price, 'num' => $amount, 'mum' => $mum, 'type' => $type, 'fee_buy' => $buy_fee, 'fee_sell' => $sell_fee, 'addtime' => time(), 'status' => 1));
				$buy_list = $mo->table('btchanges_trade')->where(array('id' => $buy['id'], 'status' => 0))->find();

				if ($buy_list) {
					if ($buy_list['num'] <= $buy_list['deal']) {
						$rs[] = $mo->table('btchanges_trade')->where(array('id' => $buy['id']))->setField('status', 1);
					}
				}

				$sell_list = $mo->table('btchanges_trade')->where(array('id' => $sell['id'], 'status' => 0))->find();

				if ($sell_list) {
					if ($sell_list['num'] <= $sell_list['deal']) {
						$rs[] = $mo->table('btchanges_trade')->where(array('id' => $sell['id']))->setField('status', 1);
					}
				}
				...
				if ($price < $buy['price']) {
					...
					if ($chajia) {
						$chajia_user_buy = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->find();
						...
						if ($chajia_save_buy_rmb) {
							$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->setDec($rmb . 'd', $chajia_save_buy_rmb);
							$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->setInc($rmb, $chajia_save_buy_rmb);
						}
					}
				}

				$you_buy = $mo->table('btchanges_trade')->where(array(
					'market' => array('like', '%' . $rmb . '%'),
					'status' => 0,
					'userid' => $buy['userid']
					))->find();
				$you_sell = $mo->table('btchanges_trade')->where(array(
				...
				if (!$you_buy) {
					$you_user_buy = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->find();

					if (0 < $you_user_buy[$rmb . 'd']) {
						$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->setField($rmb . 'd', 0);
						$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $buy['userid']))->setInc($rmb, $you_user_buy[$rmb . 'd']);
					}
				}

				if (!$you_sell) {
					$you_user_sell = $mo->table('btchanges_user_coin')->where(array('userid' => $sell['userid']))->find();

					if (0 < $you_user_sell[$xnb . 'd']) {
						$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $sell['userid']))->setField($xnb . 'd', 0);
						$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $sell['userid']))->setInc($rmb, $you_user_sell[$xnb . 'd']);
					}
				}

				$invit_buy_user = $mo->table('btchanges_user')->where(array('id' => $buy['userid']))->find();
				$invit_sell_user = $mo->table('btchanges_user')->where(array('id' => $sell['userid']))->find();
						...
						if ($buy_fee) {
							if ($invit_buy_user['invit_1']) {
								...
								if ($invit_buy_save_1) {
									...
									$rs[] = $mo->table('btchanges_invit')->add(array('userid' => $invit_buy_user['invit_1'], 'invit' => $buy['userid'], 'name' => '一代买入赠送', 'type' => $market . '买入交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_buy_save_1, 'addtime' => time(), 'status' => 1));
								}
							}
							...
							if ($invit_buy_user['invit_2']) {
								...
								if ($invit_buy_save_2) {
									$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_buy_user['invit_2']))->setInc($rmb, $invit_buy_save_2);
									$rs[] = $mo->table('btchanges_invit')->add(array('userid' => $invit_buy_user['invit_2'], 'invit' => $buy['userid'], 'name' => '二代买入赠送', 'type' => $market . '买入交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_buy_save_2, 'addtime' => time(), 'status' => 1));
								}
							}
							...
							if ($invit_buy_user['invit_3']) {
								...
								if ($invit_buy_save_3) {
									$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_buy_user['invit_3']))->setInc($rmb, $invit_buy_save_3);
									$rs[] = $mo->table('btchanges_invit')->add(array('userid' => $invit_buy_user['invit_3'], 'invit' => $buy['userid'], 'name' => '三代买入赠送', 'type' => $market . '买入交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_buy_save_3, 'addtime' => time(), 'status' => 1));
								}
							}
						}
					}
						...
						if ($sell_fee) {
							if ($invit_sell_user['invit_1']) {
								...
								if ($invit_sell_save_1) {
									$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_1']))->setInc($rmb, $invit_sell_save_1);
									$rs[] = $mo->table('btchanges_invit')->add(array('userid' => $invit_sell_user['invit_1'], 'invit' => $sell['userid'], 'name' => '一代卖出赠送', 'type' => $market . '卖出交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_sell_save_1, 'addtime' => time(), 'status' => 1));
								}
							}
							...
							if ($invit_sell_user['invit_2']) {
								...
								if ($invit_sell_save_2) {
									$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_2']))->setInc($rmb, $invit_sell_save_2);
									$rs[] = $mo->table('btchanges_invit')->add(array('userid' => $invit_sell_user['invit_2'], 'invit' => $sell['userid'], 'name' => '二代卖出赠送', 'type' => $market . '卖出交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_sell_save_2, 'addtime' => time(), 'status' => 1));
								}
							}
							...
							if ($invit_sell_user['invit_3']) {
								...
								if ($invit_sell_save_3) {
									$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $invit_sell_user['invit_3']))->setInc($rmb, $invit_sell_save_3);
									$rs[] = $mo->table('btchanges_invit')->add(array('userid' => $invit_sell_user['invit_3'], 'invit' => $sell['userid'], 'name' => '三代卖出赠送', 'type' => $market . '卖出交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_sell_save_3, 'addtime' => time(), 'status' => 1));
								}
							}
						}
					}
	...
	public function mywt($id)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables btchanges_user_coin write  , btchanges_trade write ');
		$rs = array();

		if ($trade['type'] == 1) {
			$mun = round(((($trade['num'] - $trade['deal']) * $trade['price']) / 100) * (100 + $trade_fee), 8);
			$user_buy = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->find();
			...
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->setInc($rmb, $save_buy_rmb);
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->setDec($rmb . 'd', $save_buy_rmb);
			$rs[] = $mo->table('btchanges_trade')->where(array('id' => $trade['id']))->setField('status', 2);
			$you_buy = $mo->table('btchanges_trade')->where(array(
				'market' => array('like', '%' . $rmb . '%'),
				'status' => 0,
				'userid' => $trade['userid']
			...
			if (!$you_buy) {
				$you_user_buy = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->find();

				if (0 < $you_user_buy[$rmb . 'd']) {
					$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->setField($rmb . 'd', 0);
				}
			}
		}
		else if ($trade['type'] == 2) {
			$mun = round($trade['num'] - $trade['deal'], 8);
			$user_sell = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->find();
			...
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->setInc($xnb, $save_sell_xnb);
			$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->setDec($xnb . 'd', $save_sell_xnb);
			$rs[] = $mo->table('btchanges_trade')->where(array('id' => $trade['id']))->setField('status', 2);
			$you_sell = $mo->table('btchanges_trade')->where(array(
				'market' => array('like', '%' . $xnb . '%'),
				'status' => 0,
			...
			if (!$you_sell) {
				$you_user_sell = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->find();

				if (0 < $you_user_sell[$xnb . 'd']) {
					$rs[] = $mo->table('btchanges_user_coin')->where(array('userid' => $trade['userid']))->setField($xnb . 'd', 0);
				}
			}
		}
		else {
